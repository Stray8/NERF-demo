#include<demo_http/controller_http.h>
...
void Controller::addWaypoint(){
    ROS_INFO("run into srv");
    srv.Post("/get_nbv/", [=](const Request& req, Response& res){
        auto nbv = req.body;
        Json::Reader reader;
        Json::Value root;
        if(reader.parse(nbv, root)){
            int num = root["length"].asInt();
            Json::Value nbv = root["nbv"];
            for(int i=0; i<num; i++){
                double x = nbv[i]["x"].asDouble();
                double y = nbv[i]["y"].asDouble();
                double z = nbv[i]["z"].asDouble();
                double yaw = nbv[i]["yaw"].asDouble();
                ROS_INFO_STREAM("X " << x);
                ROS_INFO_STREAM("Y " << y);
                ROS_INFO_STREAM("Z " << z);
                waypoints.push_back({x, y, z, angles::from_degrees(yaw)});
		ROS_INFO_STREAM("yaw " << angles::from_degrees(yaw));
                ROS_INFO_STREAM(waypoints.size());
            }
        }
        res.set_content("done", "text/plain");
    });
    srv.listen("192.168.31.111", 7400);
