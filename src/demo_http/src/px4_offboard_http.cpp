...
void retPose_callback(geometry_msgs::PoseStampedConstPtr msg){
    // ROS_INFO("%lf %lf %lf", msg->pose.position.x, msg->pose.position.y, msg->pose.position.z);
}

int main(int argc, char* argv[]){
    ros::init(argc, argv, "demo_cpp_http_node");
    ros::NodeHandle nh;
    Controller ctl;

    ros::Subscriber state_sub = nh.subscribe<mavros_msgs::State>("/mavros/state", 10, boost::bind(&Controller::state_callback, &ctl, _1));
    ros::Subscriber pose_sub = nh.subscribe<geometry_msgs::PoseStamped>("/vrpn_client_node/lianxu/pose", 10, boost::bind(&Controller::pos_callback, &ctl, _1));
    ...
    std::thread srv([&](){ctl.addWaypoint();});
    while(ros::ok()){
        // ROS_INFO_STREAM("current mode is : " << ctl.current_state.mode);
        if( ctl.current_state.mode == "OFFBOARD"){
            ROS_INFO("Offboard enabled");
        }
        
        if(ctl.current_state.armed){
